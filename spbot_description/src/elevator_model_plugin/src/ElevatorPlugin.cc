...
//// ----ROS--////////////////////////////////////////////////////////////////
      if (!ros::isInitialized())
     {
        int argc = 0;
        char **argv = NULL;
        ros::init(argc, argv, "elevator_client",
      ...
      // the Gazebo node
      this->rosNode.reset(new ros::NodeHandle("elevator_client"));
       std_msgs::Float32 num;
      // Create a named topic, and subscribe to it.
      ros::SubscribeOptions so =
        ros::SubscribeOptions::create<std_msgs::Float32>(
            "/" + this->model->GetName() + "/elevator_state",
            1,
            boost::bind(&ElevatorPlugin::OnRosMsg, this, _1),
            ros::VoidPtr(), &this->rosQueue);



     this->rosSub = this->rosNode->subscribe(so);
     this->rosPub = this->rosNode->advertise<std_msgs::Float32>("/button_status",100);

     rosPub.publish(num);
    // this->pub_Queue->push(std_msgs->rosPub);

      // Spin up the queue helper thread.
      this->rosQueueThread =
        std::thread(std::bind(&ElevatorPlugin::QueueThread, this));
      /////////////////////////////////////////////////
